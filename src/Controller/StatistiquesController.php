...
class StatistiquesController extends AbstractController
{
    ...
    public function evalSimpleChoisirParametresEtAfficherStats(Request $request, StatisticsManager $statsManager, Evaluation $evaluation, StatutRepository $repoStatut, GroupeEtudiantRepository $repoGroupe): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if (count($groupesChoisis) > 0 || count($statutsChoisis) > 0) {
                ...
                foreach ($groupesChoisis as $groupe) { // Groupes
                    $parametres["groupes"][] = $groupe->getId();
                }
                foreach($statutsChoisis as $statut) { // Statuts
    ...
    public function plusieursEvaluationsGroupesChoisirEvals(Request $request, StatisticsManager $statsManager, GroupeEtudiant $groupe): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $parametres = array();
            $parametres["token"] = $this->getUser()->getToken(); //Token de sécurité de l'user
            $parametres["type"] = 'plusieurs-evaluations-groupes';
            foreach ($lesGroupes as $groupe) { // Groupes
                $parametres["groupes"][] = $groupe->getId();
            }
            foreach($evaluations as $evaluation) { // Statuts
                $parametres["evaluations"][] = $evaluation->getId();
            }
            $shortURlToken = $this->getShortenedURLToken($parametres);
            $shortURl = $request->getScheme() . '://' . $request->getHttpHost() . $request->getBasePath() . $this->generateUrl('api_send_json_user', ['URLToken' => $shortURlToken]);
    ...
    public function plusieursEvaluationsStatutChoisirEvaluations(Request $request, StatisticsManager $statsManager, Statut $statut, EvaluationRepository $repoEval): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $parametres = array();
            $parametres["token"] = $this->getUser()->getToken(); //Token de sécurité de l'user
            $parametres["type"] = 'plusieurs-evaluations-statut';
            $parametres["statuts"][] = $statut->getId();
    ...
    public function evolutionGroupesChoisirEvals(Request $request, EvaluationRepository $repoEval, GroupeEtudiantRepository $repoGroupe, StatisticsManager $statsManager, GroupeEtudiant $groupe): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $parametres = array();
            $parametres["token"] = $this->getUser()->getToken(); //Token de sécurité de l'user
            $parametres["type"] = 'evolution-groupe';
    ...
    public function evolutionStatutChoisirEvals(Request $request, EvaluationRepository $repoEval, GroupeEtudiantRepository $repoGroupe, StatisticsManager $statsManager, Statut $statut): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $parametres = array();
            $parametres["token"] = $this->getUser()->getToken(); //Token de sécurité de l'user
            $parametres["type"] = 'evolution-statut';
            $parametres["statuts"][] = $statut->getId();
            foreach($lesGroupes as $groupe) {
                $parametres["groupes"][] = $groupe->getId();
            }
    ...
    public function comparaisonChoisirParametreEtAfficherStats(Request $request, StatisticsManager $statsManager, Evaluation $evaluation, StatutRepository $repoStatut, GroupeEtudiantRepository $repoGroupe): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $parametres = array();
            $parametres["token"] = $this->getUser()->getToken(); //Token de sécurité de l'user
            $parametres["type"] = 'comparaison';
            $parametres["evaluationReference"][] = $evaluation->getId();
            ...
            foreach($evaluationsChoisies as $evaluationsChoisie) {
                $parametres["autresEvaluations"][] = $evaluationsChoisie->getId();
            }
            $shortURlToken = $this->getShortenedURLToken($parametres);
            $shortURl = $request->getScheme() . '://' . $request->getHttpHost() . $request->getBasePath() . $this->generateUrl('api_send_json_user', ['URLToken' => $shortURlToken]);
